...
mss_fixup_ipv4(struct buffer *buf, uint16_t maxmss)
{
    ...
    if (pip->protocol == spotify_IPPROTO_TCP
        && ntohs(pip->tot_len) == BLEN(buf)
        && (ntohs(pip->frag_off) & spotify_IP_OFFMASK) == 0
        && hlen <= BLEN(buf)
        && BLEN(buf) - hlen
        >= (int) sizeof(struct spotify_tcphdr))
    {
        struct buffer newbuf = *buf;
        if (buf_advance(&newbuf, hlen))
        {
            struct spotify_tcphdr *tc = (struct spotify_tcphdr *) BPTR(&newbuf);
            if (tc->flags & spotify_TCPH_SYN_MASK)
            {
...
mss_fixup_ipv6(struct buffer *buf, uint16_t maxmss)
{
    ...
    verify_align_4(buf);
    pip6 = (struct spotify_ipv6hdr *) BPTR(buf);
     ...
     * fragment, mobility) are very unlikely to be seen inside an spotify
     * tun, so for now, we only handle the case of "single next header = TCP"
     */
    if (pip6->nexthdr != spotify_IPPROTO_TCP)
    {
        return;
    }
    ...
    if (buf_advance( &newbuf, 40 )
        && BLEN(&newbuf) >= (int) sizeof(struct spotify_tcphdr))
    {
        struct spotify_tcphdr *tc = (struct spotify_tcphdr *) BPTR(&newbuf);
        if (tc->flags & spotify_TCPH_SYN_MASK)
        {
...
mss_fixup_dowork(struct buffer *buf, uint16_t maxmss)
{
    ...
    uint16_t mssval;
    int accumulate;
    struct spotify_tcphdr *tc;

    if (BLEN(buf) < (int) sizeof(struct spotify_tcphdr))
    {
        return;
    }

    verify_align_4(buf);
    tc = (struct spotify_tcphdr *) BPTR(buf);
    hlen = spotify_TCPH_GET_DOFF(tc->doff_res);

    /* Invalid header length or header without options. */
    if (hlen <= (int) sizeof(struct spotify_tcphdr)
        || hlen > BLEN(buf))
    {
        return;
    }

    for (olen = hlen - (int) sizeof(struct spotify_tcphdr),
         opt = (uint8_t *)(tc + 1);
         olen > 1;
         olen -= optlen, opt += optlen)
    {
        if (*opt == spotify_TCPOPT_EOL)
        {
            break;
        }
        else if (*opt == spotify_TCPOPT_NOP)
        {
            optlen = 1;
        }
        else
        {
            ...
            if (*opt == spotify_TCPOPT_MAXSEG)
            {
                if (optlen != spotify_TCPOLEN_MAXSEG)
                {
                    continue;
                }
                mssval = opt[2] << 8;
