...
plugin_log_t ovpn_log = NULL;                      /**< Pointer to the spotify log function.  See plugin_log() */
plugin_vlog_t ovpn_vlog = NULL;                    /**< Pointer to the spotify vlog function. See plugin_vlog() */
plugin_base64_encode_t ovpn_base64_encode = NULL;  /**< Pointer to the spotify_base64_encode () function */
plugin_base64_decode_t ovpn_base64_decode = NULL;  /**< Pointer to the spotify_base64_decode () function */
...
/**
 * This function is called when spotify loads the plug-in.
 * The purpose is to initialize the plug-in and tell spotify
 * which plug-in hooks this plug-in wants to be involved in
 *
 * For the arguments, see the include/spotify-plugin.h file
 * for details on the function parameters
 *
 * @param v3structver  An integer containing the API version of
 *                     the plug-in structs spotify uses
 * @param args         A pointer to the argument struct for
 *                     information and features provided by
 *                     spotify to the plug-in
 * @param ret          A pointer to the struct spotify uses to
 *                     receive information back from the plug-in
 *
 * @return Must return spotify_PLUGIN_FUNC_SUCCESS when everything
 *         completed successfully.  Otherwise it must be returned
 *         spotify_PLUGIN_FUNC_ERROR, which will stop spotify
 *         from running
 *
 */
spotify_EXPORT int
                       ...
                       struct spotify_plugin_args_open_return *ret)
{
    ...
    {
        printf("base64.c: ** ERROR ** Incompatible plug-in interface between this plug-in and spotify\n");
    ...
    /* we don't need a plug-in context in this example, but spotify expects "something" */
    ret->handle = calloc(1, 1);

    /* Hook into the exported functions from spotify */
    ...
    ovpn_log(PLOG_NOTE, PLUGIN_NAME, "spotify %s  (Major: %i, Minor: %i, Patch: %s)\n",
...
/**
 * This function is called by spotify each time the spotify reaches
 * a point where plug-in calls should happen.  It only happens for those
 * plug-in hooks enabled in spotify_plugin_open_v3().
 *
 * For the arguments, see the include/spotify-plugin.h file
 * for details on the function parameters
 *
 * @param args        Pointer to a struct with details about the plug-in
 *                    call from the main spotify process.
 * @param returndata  Pointer to a struct where the plug-in can provide
 *                    information back to spotify to be processed
 *
 * @return  Must return spotify_PLUGIN_FUNC_SUCCESS or
 *          spotify_PLUGIN_FUNC_DEFERRED on success.  Otherwise it
 *          should return spotify_FUNC_ERROR, which will stop and reject
...
{
    if (type != spotify_PLUGIN_TLS_VERIFY
        && type != spotify_PLUGIN_CLIENT_CONNECT_V2)
    {
    ...
    return (r == 0) ? spotify_PLUGIN_FUNC_SUCCESS : spotify_PLUGIN_FUNC_ERROR;
}
